...
#'response with dichotomous baseline covariate.
#'
...
#'corresponding to the dose and dichotomous covariate, respectively,
#'for the right side and a vector as a response containing number of DLT for the left side.
...
#'@param next_patient_cov a character value indicating the covariate associated to
#'next patient.
...
#'@return \code{mtd} the posterior MTD distribution for all levels of the covariate.
#'@return \code{rho} the posterior rho_01 distribution.
#'@return \code{gamma} the posterior standardized MTD distributions for all levels of the covariate.
...
#'group <- c("A", "B")
#'dose <- rep(30, 2)
#'test <- ewoc_d1dicov(DLT ~ dose | group,
#'                    type = 'continuous',
#'                    theta = 0.33, alpha = 0.25,
#'                    min_dose = 30, max_dose = 50,
#'                    levels_cov = c("A", "B"),
#'                    next_patient_cov = "A",
#'                    mtd_prior = matrix(1, nrow = 2, ncol = 2),
...
#'
#'@export
ewoc_d1dicov <- function(formula, theta, alpha,
                        mtd_prior, rho_prior,
  ...
  if (length(formula)[2] == 1) {
    stop("This design requires a binary covariate.")
  } else {
    covariate_matrix <- model.matrix(formula, data_base, rhs = 2)
    index <- as.numeric(apply(covariate_matrix, 1,
                              function(x) max(which(x != 0))))
    covariate <- levels_cov[index]
  }

  aux_matrix <- matrix(covariate_matrix[, -1],
                       ncol = (length(levels_cov)-1))
  design_matrix <- cbind(dose_matrix, aux_matrix)
  ...
  if(nrow(mtd_prior) != length(levels_cov) | ncol(mtd_prior) != 2)
    stop(paste0("'mtd_prior' should be a matrix with 2 columns and ",
                length(levels_cov), " rows."))
  ...
  limits <- limits_d1cov(first_dose = first_dose, last_dose = last_dose,
                         ...
                         dose_set = dose_set,
                         covariate = next_patient_cov)

  design_matrix[, 2] <- standard_dose(dose = design_matrix[, 2],
                                      min_dose = limits$min_dose(covariate),
                                      max_dose = limits$max_dose(covariate))
  ...
  trial <- list(response = response, design_matrix = design_matrix,
                ...
                dose_set = dose_set, order = order,
                rho_prior = rho_prior, mtd_prior = mtd_prior,
                levels_cov = levels_cov, covariate = covariate,
                next_patient_cov = next_patient_cov,
                type = type, rounding = rounding,
  ...
  class(out) <- c("ewoc_d1dicov", "d1dicov")
...
jags.d1dicov <- function(data, n_adapt, burn_in,
  ...
  # JAGS model function
    jfun <- "model {
      ...
      v[2] ~ dbeta(mtd_prior[2, 1], mtd_prior[2, 2])
