#'Generic CRM simulation
#'
#'Generic function for simulating CRM trials.
#'
#'@param step_zero an object from the classes 'crm_d1classic', 'crm_d1extended',
...
#'
#'@export
crm_simulation <- function(step_zero, n_sim, sample_size,
                           response_sim,
                           stop_rule_sim = NULL,
                           ncores = 1){
  UseMethod("crm_simulation")
}


#'@export
crm_simulation.crm_d1classic <- function(step_zero, n_sim, sample_size,
                                         response_sim = NULL,
                                         stop_rule_sim = NULL,
                                         ncores = 1){
  ...
  result <-
    foreach(i = 1:n_sim,
            ...
            .init=list(list(), list(), list(), list())) %dopar% {
              ...
              list(dose, dlt, mtd_estimate, rho_estimate)
            }
  stopImplicitCluster()
  ...
  out <- list(dose_sim = dose_sim, dlt_sim = dlt_sim,
              mtd_sim = mtd_sim, rho_sim = rho_sim)
  return(out)
}
...
#'@importFrom doParallel registerDoParallel
#'@export
crm_simulation.crm_d1extended <- function(step_zero, n_sim, sample_size,
                                          response_sim = NULL,
                                          stop_rule_sim = NULL,
                                          ncores = 1){
...
#'@export
crm_simulation.crm_d1ph <- function(step_zero, n_sim, sample_size,
                                    response_sim = NULL,
                                    stop_rule_sim = NULL,
                                    ncores = 1){
  ...
  result <-
    foreach(i = 1:n_sim,
            ...
            .init=list(list(), list(), list(), list(), list())) %dopar% {
              ...
              list(event_time, dose, dlt, mtd_estimate, rho_estimate)
            }
  stopImplicitCluster()
