...
#'response with binary covariable.
#'
...
#'group <- c("A", "B")
#'dose <- rep(30, 2)
#'test <- ewoc_d1dcov(DLT ~ dose | group,
#'                    type = 'continuous',
...
#'
#'@export
ewoc_d1dcov <- function(formula, theta, alpha,
                        mtd_prior, rho_prior,
  ...
  if (length(formula)[2] == 1) {
    stop("This design requires a binary covariable.")
  } else {
  ...
  class(out) <- c("ewoc_d1dcov", "d1dcov")
...
#'@export
ewoc_jags.d1dcov <- function(data, n_adapt, burn_in,
